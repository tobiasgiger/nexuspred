...
export default {
  ...
  render(root, { navigate }) {
    ...
    function paintSelect(list) {
      ...
      sel.replaceChildren();
      if (!list.length) {
        sel.append(h("option", { value: "" }, t("No webhooks — create one first")));
      } else {
    ...
    function paintTemplate() {
      ...
      const tpl = alertMessageTemplate(w.strategy);
    ...
    if (can(me, "discord")) {
      ...
      dsCard = card({ title: t("Test Discord signal"), hint: t("Runs a synthetic Discord embed through the listener pipeline for a channel (parse → dispatch), respecting the dry-run switch — verify fan-out without a live Discord connection.") },
        h("div", { class: "grid grid-2" },
          h("div", { class: "field" }, h("label", null, t("Channel ID")), chanInp),
          h("div", { class: "field" }, h("label", null, t("Preset")), presetSel)),
        h("div", { class: "field" }, h("label", null, t("Embed (JSON: title + fields)")), embedTa),
    ...
    root.append(
      pageHead(t("Tools"), t("Everything you need to wire TradingView and Discord to the bridge, and to test it safely.")),
      card({ title: t("Webhook URL & alert message") },
        h("div", { class: "field", style: "max-width:420px" }, h("label", null, t("Webhook")), sel),
        h("p", { class: "hint" }, t("Point the TradingView alert's Webhook URL (POST, JSON body) at:")),
        h("div", { class: "url-box" }, urlCode, copyButton(() => urlCode.textContent)),
        h("h3", null, t("Alert message")),
        h("p", { class: "hint" }, t("Paste into the TradingView alert's \"Message\" box.")),
        tmplPre, h("div", { class: "form-actions" }, copyButton(() => tmplPre.textContent, "Copy message", "btn btn-secondary btn-sm")), tmplHint,
        h("div", { class: "form-actions" }, h("button", { type: "button", class: "btn btn-ghost", onClick: () => navigate("/webhooks") }, t("Manage webhooks "), icon("chevron")))),
      card({ title: t("Send test signal"), hint: t("Runs the full live pipeline for the selected webhook — real execution if Trading is enabled. Load an example, then send.") },
      ...
      dsCard,
      card({ title: t("Browser token extractor (Chrome / Edge)"), actions: [h("a", { class: "btn btn-primary", href: "/api/extension/token-extractor.zip", download: "" }, icon("download"), t("Download extension (.zip)"))] },
        h("p", { class: "hint" }, t("A small browser extension that reads your own tokens from a logged-in tab — the Discord user token (Settings → Discord Listener) and the Tradovate token + checkToken (Settings → Broker Accounts). It runs 100% locally and never sends anything anywhere. Each token is like a password for that account — only paste it back into this bridge.")),
        h("h3", null, t("Install (one time)")),
        h("ol", { class: "steps" },
          h("li", null, h("span", { class: "step-num" }, "1"), h("div", null, h("strong", null, t("Download")), t(" the .zip above and unzip it — you get a "), h("code", null, "token-extractor"), t(" folder."))),
          h("li", null, h("span", { class: "step-num" }, "2"), h("div", null, t("Open "), h("code", null, t("chrome://extensions")), t(" (or "), h("code", null, t("edge://extensions")), t(") and turn on "), h("strong", null, t("Developer mode")), ".")),
          h("li", null, h("span", { class: "step-num" }, "3"), h("div", null, t("Click "), h("strong", null, t("Load unpacked")), t(" and select the unzipped folder.")))),
        h("h3", null, t("Use")),
        h("ol", { class: "steps" },
          h("li", null, h("span", { class: "step-num" }, "1"), h("div", null, t("Open "), h("a", { href: "https://discord.com/app", target: "_blank", rel: "noopener" }, "discord.com"), t(" or the "), h("a", { href: "https://trader.tradovate.com", target: "_blank", rel: "noopener" }, t("Tradovate web trader")), t(", logged in."))),
          h("li", null, h("span", { class: "step-num" }, "2"), h("div", null, t("Click the extension icon → "), h("strong", null, t("Extract from active tab")), t(" → Reveal / Copy."))),
          h("li", null, h("span", { class: "step-num" }, "3"), h("div", null, t("Paste into the matching field here and save.")))),
        h("p", { class: "hint" }, t("After a bridge update, re-download and reload the extension (↻ in chrome://extensions) to pick up changes."))),
      card({ title: t("No-install option: bookmarklets"), hint: t("Drag a button to your bookmarks bar once, then click it while on the site (logged in) to copy the token. Discord's strict security policy can block bookmarklets — if the Discord one does nothing, use the extension. Tradovate works reliably.") },
