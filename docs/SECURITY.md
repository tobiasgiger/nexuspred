...
| Two-factor | TOTP (RFC 6238, SHA-1/6 digits/30 s, ±1 step) enforced for every account created through sign-up or first-run setup, optional for older ones; codes are single-use (replay counter); ten backup codes, salted-hashed, deleted on use, renewable at any time with password + code; the secret is Fernet-encrypted; the "password verified, code outstanding" state is a separate 5-minute signed token that `read_session` refuses; `/login/2fa` and `/2fa/setup` are rate-limited and the per-account brake counts second-factor failures; admin reset (Users page) and password reset both wipe the secret and codes, revoke every session and force re-enrolment. |
