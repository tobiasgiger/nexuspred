"""Translate incoming TradingView webhook signals into broker orders.

Every signal arrives through a specific webhook with its own secret URL,
strategy and routed trade accounts. This module validates and authorises the
signal, applies entry-only locks/windows, serialises management for one trade,
and delegates broker mutations to :mod:`app.engine`.
"""
...
def forward_to_subscribers(payload: dict[str, Any], webhook: dict[str, Any],
    ...
    """Fan out only to subscriptions still authorised by the current publisher ACL."""
...
async def flatten_all() -> dict[str, Any]:
    """Emergency kill-switch across all enabled logins in the current area."""
    mgr = manager()
    mgr.reload()
    executors: list[AccountExecutor] = []
    for session in mgr.all():
        if not session.enabled:
            continue
        for account in session.accounts:
            executors.append(AccountExecutor(session, account))
    if not executors:
    ...
    state.log_event(
        "warn",
        f"🆘 SOS flatten-all: {flattened} position(s) flattened, {cancelled} order(s) cancelled across {len(executors)} account(s)"
