...
news-lock rules — without any secret. Marketplace authorization is deliberately
...
def _portable_webhook(raw: Any) -> Any:
    """Strip installation-local publication authority from an exported webhook."""
...
def _import_webhook(w: dict[str, Any], current: dict[str, Any], area_id: int) -> dict[str, Any]:
    """Normalise an imported webhook without importing local authorization.

    Webhook routing is retained only for stable login ids present on this bridge.
    Marketplace publication is always off after import and selected-user ids are
    dropped; an administrator must deliberately publish it on this installation.
    """
    try:
